...
class Ecg_preprocess():
    ...
    def tailor_rpeaks_length(self, data, rpeaks):
    ...
    def Q_S_detector(self, rpeaks, Q_dict, S_dict):
        ...
        if len(S) == rpeaks.size + 1:
            for i in range(1, rpeaks.size):
               if rpeaks[i] > clean_s[i - 1] and (rpeaks[i] < clean_s[i]):
                   new_S.append(clean_s[i])

               elif rpeaks[i] > clean_s[i] and (rpeaks[i] < clean_s[i + 1]):
                   new_S.append(clean_s[i+1])
